...
OUTPUT_PATH = "/Users/macintoshhd/Downloads/MSE24/2025_semester_02/MLE501.9/final/annotated_chess_moves.mp4"

def load_yolo_model(model_path=MODEL_PATH):
    """Load the pre-trained YOLOv8 model for piece detection."""
    piece_classes = [
...
def predict_piece(square_img, img_name, model, class_indices, frame_idx, threshold=0.6, debug=False):
    """Predict chess piece in a square image using the YOLOv8 model."""
    if square_img.size == 0 or square_img.shape[0] == 0 or square_img.shape[1] == 0:
        logging.warning(f"Empty square image: {img_name}")
        return None

    # Ensure image is in RGB for YOLOv8
    square_rgb = cv2.cvtColor(square_img, cv2.COLOR_GRAY2RGB) if len(square_img.shape) == 2 else square_img
    square_resized = cv2.resize(square_rgb, (80, 80))
    square_resized = blur_border(square_resized, border_size=6)
    
    # Run YOLOv8 prediction with MPS for Apple Silicon
    try:
        results = model.predict(square_resized, imgsz=80, conf=threshold, verbose=False, device="mps")
        detections = results[0].boxes  # Get bounding boxes
        
        if len(detections) == 0:
            if debug:
                logging.debug(f"No detections in {img_name}")
            return None
        ...
        for box in detections:
            ...
            piece = idx_to_piece.get(class_idx, "empty")
            if conf > max_conf and piece != "empty":
                max_conf = conf
                predicted_piece = piece
        
        if debug and predicted_piece:
            ...
            annotated_img = results[0].plot()  # YOLOv8's built-in visualization
            cv2.imwrite(f'{frame_debug_dir}/{img_name}_detection.png', annotated_img)
            with open(f'{frame_debug_dir}/{img_name}_prediction.txt', 'w') as f:
                f.write(f"Predicted: {predicted_piece}, Confidence: {max_conf:.2f}\n")
            match_result.append(f"{img_name} predicted = {predicted_piece} ({max_conf:.2f})")
        
        if predicted_piece and max_conf >= threshold:
            return predicted_piece
        return None
    except Exception as e:
        logging.warning(f"YOLOv8 prediction failed for {img_name}: {e}")
        return None
    ...
    for row in range(8):
        for col in range(8):
            ...
            if not square.size or square.shape[0] < dy or square.shape[1] < dx:
                ...
                continue
            squares.append(square)  # Keep RGB for YOLOv8
            name = f'square_r{row}_c{col}.png'
            debug_path = os.path.join(debug_dir, name)
            cv2.imwrite(debug_path, square)
...
def generate_fen(squares, square_names, model, class_indices, frame_idx, debug=False):
    ...
    fen_map = {
        'WhitePawn': 'P', 'WhiteRook': 'R', 'WhiteKnight': 'N', 'WhiteBishop': 'B',
        'WhiteQueen': 'Q', 'WhiteKing': 'K', 'BlackPawn': 'p', 'BlackRook': 'r',
        'BlackKnight': 'n', 'BlackBishop': 'b', 'BlackQueen': 'q', 'BlackKing': 'k'
    }
...
def main(start_time=200, end_time=350, frame_interval=1):
    ...
    try:
        ...
        fourcc = cv2.VideoWriter_fourcc(*'avc1')  # Use 'avc1' for macOS compatibility
        ...
        model, class_indices = load_yolo_model()
