...
IMG_SIZE = 80  # Match original script's image size
NUM_CLASSES = 13  # WhitePawn, WhiteRook, ..., BlackKing, empty

def download_dataset():
    """Download and extract the Roboflow dataset."""
    try:
        os.makedirs(DATASET_DIR, exist_ok=True)
        # Run curl command to download and extract dataset
        subprocess.run(
...
def verify_dataset(dataset_dir=DATASET_DIR):
    """Verify the dataset structure and data.yaml."""
    ...
    try:
        with open(data_yaml, 'r') as f:
            ...
            if data_config['nc'] != NUM_CLASSES:
                ...
                # Optionally, update data.yaml to match expected classes
                data_config['nc'] = NUM_CLASSES
                data_config['names'] = [
                ...
                logging.info("Updated data.yaml to match %d classes", NUM_CLASSES)
