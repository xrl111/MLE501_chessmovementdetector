...
# Constants
INITIAL_IMAGE_LIMIT = 20  # Minimum number of images to process before balancing
MAX_FILES_PER_CLASS = 100  # Maximum files per class
...
def preprocess_dataset(dataset_path, output_path, class_names, custom_dataset_path="custom_dataset", max_files_per_class=MAX_FILES_PER_CLASS):
    ...
    class_to_folder = {cls: f"{i}_{cls}" for i, cls in enumerate(class_names)}
    ...
    if dataset_path and os.path.exists(dataset_path):
        ...
        for split in ['train', 'valid']:
            ...
            for img_file in image_files:
                ...
                with open(label_path, 'r') as f:
                    lines = f.readlines()  # Fixed: Removed extra ']'
                for i, line in enumerate(lines):
                    ...
                    try:
                        ...
                        save_name = f"orig_{img_file.rsplit()[0]}_{i}.jpg"
        ...
        if processed_images >= INITIAL_IMAGE_LIMIT and not all_files_processed:
            for split in ['train', 'valid']:
                image_dir = os.path.join(dataset_path, split, 'images')  # Fixed: Corrected path
                label_dir = os.path.join(dataset_path, split, 'labels')  # Fixed: Corrected path
                if not os.path.exists(image_dir) or not os.path.exists(label_dir):
                    logger.warning(f"Missing image or label directory for {split}")
                    continue
                image_files = [f for f in os.listdir(image_dir) if f.lower().endswith(('.jpg', '.png'))]
    ...
    if os.path.exists(custom_dataset_path) and not all_files_processed:
        ...
        for file_name in os.listdir(custom_dataset_path):
            ...
            for aug_idx in range(aug_count):
                ...
                remapped_class_counts[target_class] += 1
            processed_images += 1
            processed_custom += 1  # Increment processed_custom
            if processed_images % 10 == 0:
                logger.info(f"Processed {processed_images} images")
            del img
            gc.collect()
...
def main():
    """Main function to download, inspect, and preprocess the chess dataset."""
    working_dir = os.getcwd()
    dataset_path = os.path.join(working_dir, "dataset")
    output_path = os.path.join(working_dir, "chess_classification_dataset")
    custom_dataset_path = os.path.join(working_dir, "custom_dataset")
